    ...
    <div style={{ width: 300, maxWidth: '100%', margin: '0 auto', aspectRatio: '1 / 1', display: 'grid', placeItems: 'center' }}>
      <button id="speedtest-go" className="go-btn" onClick={onGo} aria-label="Start speed test">
        <svg viewBox="0 0 300 300" style={{ width: '100%', height: '100%', display: 'block' }}>
          ...
          <text x="150" y="165" textAnchor="middle" fontSize="44" fontWeight="700" fill="#2563eb" letterSpacing="5" fontFamily="var(--font-main)">GO</text>
        </svg>
      </button>
    </div>
  )
...
function SpeedGauge({ value, phase }) {
  const { cx, cy, r } = GAUGE
  const color = PHASE_COLOR[phase] || '#2563eb'
  ...
  return (
    <div style={{ width: 300, maxWidth: '100%', margin: '0 auto', aspectRatio: '1 / 1' }}>
      <svg viewBox="0 0 300 300" style={{ width: '100%', height: '100%', display: 'block' }}>
        ...
        {/* Track */}
        <path d={arcPath(0, 1)} fill="none" stroke="rgba(15,36,68,0.10)" strokeWidth="14" strokeLinecap="round" />
        ...
        {Array.from({ length: 41 }).map((_, i) => {
          ...
          return <line key={i} x1={inn.x} y1={inn.y} x2={out.x} y2={out.y}
            stroke={major ? 'rgba(15,36,68,0.45)' : 'rgba(15,36,68,0.18)'} strokeWidth={major ? 2 : 1} strokeLinecap="round" />
        })}
        ...
        <text x={cx} y={cy + 42} textAnchor="middle" fill="#0f2444" fontSize="32" fontWeight="800" fontFamily="var(--font-mono)">
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={160}>
        <AreaChart data={data} margin={{ top: 6, right: 12, left: 0, bottom: 0 }}>
          ...
          <Tooltip contentStyle={{ background: 'rgba(255,255,255,0.97)', border: `1px solid ${color}66`, borderRadius: '10px', fontSize: '13px' }} labelStyle={{ color: '#94a3b8' }} />
    ...
    <div className="result-panel">
      <button className="go-btn result-go" onClick={onRetest} aria-label="Test again">
        <svg viewBox="0 0 120 120" style={{ width: '100%', height: '100%', display: 'block' }}>
          ...
          <text x="60" y="68" textAnchor="middle" fontSize="22" fontWeight="700" fill="#2563eb" letterSpacing="2" fontFamily="var(--font-main)">GO</text>
        </svg>
      </button>
      ...
      <div className="result-metrics">
        <div className="result-speeds">
          ...
          <div className="result-speed">
            <div className="result-cap"><span style={{ color: '#4f46e5' }}>↑</span> UPLOAD <em>Mbps</em></div>
            ...
            return (
              <div style={{ marginTop: '24px' }}>
                ...
                <div className="two-col">
                  {dlData.length > 0 &&
                    <MiniChart title="↓ DOWNLOAD" unitLabel="Mbps" data={dlData} color="#2563eb" gradId="dlGrad" />}
                  {ulData.length > 0 &&
                    <MiniChart title="↑ UPLOAD" unitLabel="Mbps" data={ulData} color="#4f46e5" gradId="ulGrad" />}
                </div>
              </div>
            )
          })()}
          ...
          <div className="input-row" style={{ display: 'flex', gap: '12px', marginBottom: '16px', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end' }}>
              <button className="btn btn-primary" onClick={handlePing} disabled={pinging}
                style={{ height: '46px', background: 'var(--cyan)', color: '#ffffff' }}>
                {pinging ? <><div className="spinner" style={{ borderTopColor: '#ffffff' }} />&nbsp;Pinging…</> : '◍ Run Ping'}
              </button>
            </div>
          </div>
