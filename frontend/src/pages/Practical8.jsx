  ...
  useEffect(() => {
    ...
    const open = () => {
      ...
      ws.onmessage = (ev) => {
        ...
        if (data.connected !== false && data.rssi != null) {
          setRolling(prev => {
            const next = [...prev, { t: new Date(data.timestamp).toLocaleTimeString(), rssi: data.rssi }]
            return next.length > 60 ? next.slice(next.length - 60) : next
          })
          if (recordingRef.current) samplesRef.current.push(data.rssi)
        }
      }
      ws.onclose = () => {
        if (stopped) return
  ...
  return (
    <main className="practical-page">
      <div className="container">
        ...
        {/* ── LIVE PANEL ── */}
        <div className="glass-card" style={{ marginBottom: '24px', borderColor: connected ? 'rgba(245,158,11,0.2)' : 'var(--border)' }}>
          ...
          {connected && (
            <div className="four-col">
              <div className="stat-pill">
                <div style={{ fontSize: '13px', fontWeight: '700', color: 'var(--amber)', marginBottom: '4px', fontFamily: 'var(--font-mono)' }}>{liveWifi.ssid}</div>
        ...
        <div className="two-col" style={{ marginBottom: '24px' }}>
          <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
            <div>
              <div style={{ fontSize: '16px', fontWeight: '700', marginBottom: '16px', color: 'var(--amber)' }}>🧪 Multipath Experiment Setup</div>
            ...
            {!recording ? (
              <button className="btn btn-primary" onClick={startSession} disabled={!connected}
                style={{ background: 'var(--amber)', color: '#050a18' }}>
          ...
          <div className="glass-card">
            <div style={{ fontSize: '16px', fontWeight: '700', marginBottom: '16px', color: 'var(--amber)' }}>📊 Fading Statistics</div>
            {latest ? (
              <>
                <div className="three-col" style={{ marginBottom: '14px' }}>
                  ...
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: '22px', fontWeight: 800, fontFamily: 'var(--font-mono)', color: 'var(--amber)' }}>±{latest.std_dev}</div>
        ...
        <div className="glass-card" style={{ marginBottom: '24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px', flexWrap: 'wrap', gap: '10px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button className={`btn btn-sm ${tab === 'realtime' ? 'btn-primary' : 'btn-outline'}`}
                style={tab === 'realtime' ? { background: 'var(--amber)', color: '#050a18' } : {}}
                onClick={() => setTab('realtime')}>Rolling</button>
              <button className={`btn btn-sm ${tab === 'distribution' ? 'btn-primary' : 'btn-outline'}`}
                style={tab === 'distribution' ? { background: 'var(--amber)', color: '#050a18' } : {}}
          ...
          {tab === 'realtime' ? (
            rolling.length === 0 ? (
              <div className="empty-state"><div className="empty-state-icon">📡</div><div className="empty-state-text">Waiting for live signal…</div></div>
            ) : (
              <ResponsiveContainer width="100%" height={320}>
                <LineChart data={rolling} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
                  ...
                  <Tooltip contentStyle={{ background: 'rgba(8,13,32,0.95)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: '10px', fontSize: '13px' }} />
                  ...
                  <Line type="monotone" dataKey="rssi" name="Live RSSI" stroke="#f59e0b" strokeWidth={2} dot={false} isAnimationActive={false} />
                </LineChart>
              </ResponsiveContainer>
            )
          ) : (
            <ResponsiveContainer width="100%" height={320}>
              <BarChart data={latest?.distribution || []} margin={{ top: 10, right: 20, left: 0, bottom: 16 }}>
                ...
                <Bar dataKey="measured" name="Measured (your data)" fill="#f59e0b" radius={[4, 4, 0, 0]} />
                <Line dataKey="rayleigh" name="Rayleigh (fitted)" type="monotone" stroke="#00d4ff" strokeWidth={2} dot={false} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
              ...
              <table className="data-table">
                ...
                <tbody>
                  {sessions.map((s, i) => (
                    <tr key={s.id}>
                      <td>{i + 1}</td>
                      <td style={{ color: 'var(--amber)' }}>{s.scenario}</td>
