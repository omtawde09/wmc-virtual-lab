...
def get_path_loss_fit():
    ...
    (regardless of obstacle count) and returns the model parameters plus
    a few example distance predictions.
    """
...
def get_obstacle_analysis():
    """
    Groups logged readings by obstacle_count and reports the average RSSI
    and attenuation (in dB) relative to the fewest-obstacle group, plus an
    overall dB-per-obstacle estimate via linear regression across all
    readings. This isolates the obstacle effect from the pure distance
    effect - answering "how much does a wall cost me", independent of
    exactly how far away the reading was taken.
    """
    if not range_readings:
        raise HTTPException(
            status_code=409,
            detail="No readings logged yet. Log readings with an obstacle_count "
                   "via POST /api/bluetooth/reading first.",
        )

    groups: dict = {}
    for r in range_readings:
    ...
    interpretation = None
    if len(groups) >= 2:
        xs = np.array([r.get("obstacle_count", 0) for r in range_readings], dtype=float)
        ys = np.array([r["rssi"] for r in range_readings], dtype=float)
